...
let BiasDetectionService = class BiasDetectionService {
    supabase;
    log = new common_1.Logger('BiasDetection');
    constructor(supabase) {
        this.supabase = supabase;
    }
    async analyzeUser(userId, userProfile) {
        ...
        try {
            ...
            this.log.warn('Bias analysis error: ' + err.message);
            return empty;
        }
    }
    scoreRecency(events) {
    ...
    scoreFamiliarity(events) {
        const withSector = events.filter(e => e.sector && e.sector !== 'cognitive');
        if (withSector.length < 5)
            return 0;
        const counts = {};
        for (const e of withSector)
            counts[e.sector] = (counts[e.sector] ?? 0) + 1;
        const max = Math.max(...Object.values(counts));
        return Math.min(max / withSector.length, 1);
    }
    scoreLossAversion(events) {
        const simEvents = events.filter(e => e.event_type === 'simulator_used' && e.metadata?.result_pct !== undefined);
        ...
        if (pos === 0)
            return Math.min(neg / 4, 0.9);
        const ratio = neg / pos;
        return ratio > 2 ? Math.min(ratio / 5, 1) : 0;
    }
    scoreOverconfidence(profile) {
    ...
    scoreDisposition(events) {
        ...
        if (sellIntent + holdIntent === 0)
            return 0;
        if (holdIntent === 0)
            return Math.min(sellIntent / 5, 0.8);
        const ratio = sellIntent / holdIntent;
        return ratio > 2.5 ? Math.min(ratio / 5, 0.9) : 0;
    }
    buildEvidence(key, events, profile) {
        switch (key) {
            case 'recency': {
                const n = events.filter(e => e.event_type === 'recommendation_requested' &&
                    Date.now() - new Date(e.created_at).getTime() < 86_400_000).length;
            ...
            case 'familiarity': {
                ...
                if (!top)
                    return 'Alta concentración en un sector específico del mercado.';
            ...
            case 'overconfidence': {
                ...
                let suggested = 5;
                if (profile?.age_group === '55+')
                    suggested -= 2;
                else if (profile?.age_group === '45-54')
                    suggested -= 1;
                else if (profile?.age_group === '18-24')
                    suggested += 1;
                if (profile?.monthly_income === 'less_than_1m')
                    suggested -= 2;
                suggested = Math.max(1, Math.min(10, suggested));
...
exports.BiasDetectionService = BiasDetectionService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [supabase_service_1.SupabaseService])
], BiasDetectionService);
//# sourceMappingURL=bias-detection.service.js.map