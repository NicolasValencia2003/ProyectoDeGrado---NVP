...
exports.buildLifeProfileContext = buildLifeProfileContext;
exports.buildBiasInstructions = buildBiasInstructions;
function buildLifeProfileContext(user, prefs) {
    const lines = [];
    const ageContexts = {
    ...
    if (mlStrength > 0 && prefs?.sector_weights) {
        ...
        const avoided = prefs.avoided_tickers ?? [];
        if (avoided.length > 0) {
...
function buildBiasInstructions(detectedBiases) {
    if (!detectedBiases || Object.keys(detectedBiases).length === 0)
        return '';
    const instructions = Object.values(detectedBiases)
        .filter((b) => b?.detected && b?.prompt_instruction)
        .map((b) => b.prompt_instruction)
        .join('\n');
    return instructions ? `\nBEHAVIORAL BIAS INSTRUCTIONS:\n${instructions}\n` : '';
}
//# sourceMappingURL=life-profile.service.js.map